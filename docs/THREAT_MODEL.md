...
|---|---|---|
| Desktop-WebView | Konfiguration, Start/Stop, Codeanzeige und -rotation, Sitzungswiderruf, Firewall und Diagnose | `src-tauri/src/lib.rs:385-550`, `src-tauri/src/lib.rs:581-630` |
...
|---|---|---|---|---|---|---|
| Laufender LAN-Dienst | Listener | ausgewählte Schnittstelle plus gespeicherter Port | exakt `<private IPv4>:<Port>` | LAN-Clients im zugehörigen Subnetz | Bindung, Accept-Prüfung, Request-Subnetz-, Host- und Originprüfung | `src-tauri/src/service/mod.rs:360-386`, `src-tauri/src/service/mod.rs:436-447`, `src-tauri/src/service/api/common.rs:45-121` |
...
| Download | konfigurierte Downloadwurzel | lokale Einstellung beim Start; vollständig getrennt von Upload | kanonisierter, geöffneter Rootanker | authentisierte Browser lesen einzelne Dateien | read-only Handles, Enthaltensein, Reparse-/Namespace-Prüfung und Downloadlimits | `src-tauri/src/domain/shares.rs:58-104`, `src-tauri/src/service/api/download.rs:212-268` |
| Upload | konfigurierte Uploadwurzel und `.dmdc` | lokale Einstellung beim Start; nur lokales zulässiges Volume | kanonisierte Wurzel; serverseitige Partials und opake Zielnamen | authentisierte Browser schreiben neue Dateien; Rust veröffentlicht | kein Listing, Sitzungsbesitz, CSRF, exakter Offset, Budgets, Autoload-Sperren und No-Replace | `src-tauri/src/domain/shares.rs:221-249`, `src-tauri/src/service/api/upload.rs:224-333`, `src-tauri/src/service/api/upload.rs:467-495` |
