...
- Upload: Metadaten- und begrenzte Speicherprüfung → neue `.part`-Datei → frühe Besitzprüfung vor PATCH-Bodyverbrauch → exakt 8 MiB pro Zwischenblock und genau die verbleibende Größe im letzten Block am jeweils bestätigten Offset → Größen- und Datenträgersynchronisation → dienstbesessener Commit → atomare No-Replace-Umbenennung auf einen immer zufällig suffigierten Zielnamen. Hat der Commit linearisiert, warten Abbruch, Widerruf und Stop auf dessen Ergebnis; das Verschwinden des HTTP-Waiters trennt den Commit nicht ab.
